...
@section('content')
    @include('layouts.headers.cards-ig')
        ...
        <div class="row">
            <div class="col-xl-8 mb-2 mb-xl-0">
                <div class="card bg-gradient-white shadow card-tab">
                    ...
                    <div class="card-body">
                        <!-- Chart -->
                        <div class="chart">
                            @include('layouts.chart.dashboard-line-ig')
                        </div>
                    </div>
                </div>
                @php
            ...
            <div class="col-xl-4">
                <div class="card shadow">
                    <div class="card-body">
                        <h2 class="text-default mb-0 text-center">Total Followers</h2>
                        <hr class="my-3">
    ...
    <script>
        ...
        function loadData(date = null) {
            $.ajax({
                ...
                success: function(r) {
                    // card
                    $('#followers').text(r.card.followers);
                    $('#pfollowers').text(r.persentage.followers + ' %');
                    $('#likes').text(r.card.likes);
                    $('#plikes').text(r.persentage.likes + ' %');
                    $('#comments').text(r.card.comments);
                    $('#pcomments').text(r.persentage.comments + ' %');
                    $('#message').text(r.card.message);
                    $('#pmessage').text(r.persentage.message + ' %');
                    ...
                    $('#dmy-d-data').text(d.followers);
                    $('#dmy-m-data').text(m.followers);
                    $('#dmy-y-data').text(y.followers);

                    $('#dmy-d-percent').text(get_percentage(d.followers, d.followers - pd.followers) + "%");
                    $('#dmy-m-percent').text(get_percentage(m.followers, m.followers - pm.followers) + "%");

                    if (py.length) {
                        py = py[0];
                        $('#dmy-y-percent').text(get_percentage(y.followers, y.followers - py.followers) + "%");
                    }
                }
            });
        }
